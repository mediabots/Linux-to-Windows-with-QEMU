...
link1_status=$(curl -Is https://app.vagrantup.com/thuonghai2711/boxes/WindowsISO/versions/1.0.6/providers/qemu.box | grep HTTP | cut -f2 -d" " | head -1)
...
if [ $link1_status = "302" ] ; then 
	sudo wget -O /mediabots/WS11LITE.ISO https://app.vagrantup.com/thuonghai2711/boxes/WindowsISO/versions/1.0.6/providers/qemu.box
...
User: Administrator
Pass: Thuonghai001
Connect to VNC, choose disk for windows install when setup done use RDP to connect
sleep 10
